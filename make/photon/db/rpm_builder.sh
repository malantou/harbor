...
wget -e "https_proxy=https://127.0.0.1:7890" http://ftp.postgresql.org/pub/source/v$version/$name-$version.tar.bz2
...
wget -e "https_proxy=https://127.0.0.1:7890" https://raw.githubusercontent.com/vmware/photon/4.0/tools/scripts/build_spec.sh
sed "s|VERSION=3|VERSION=4|g" -i build_spec.sh
chmod 655 ./build_spec.sh && cp $cur/postgres.spec .
./build_spec.sh ./postgres.spec
cp ./stage/RPMS/aarch64/${name}96-libs-$version-1.ph4.aarch64.rpm $cur
cp ./stage/RPMS/aarch64/${name}96-$version-1.ph4.aarch64.rpm $cur
